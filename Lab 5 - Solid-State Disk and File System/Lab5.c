...
//*******attach background tasks***********
  ...
  OS_AddPeriodicThread(disk_timerproc,10*TIME_1MS,3, 1);
...
void TestDisk(void){  DSTATUS result;  unsigned short block;  int i; unsigned long n;
  ...
  for(block = 0; block < MAXBLOCKS; block++){
    ...
    if(eDisk_WriteBlock(buffer,block))diskError("eDisk_WriteBlock\n",block); // save to disk
    GPIO_PF3 = 0x00;     
  }  
...
void RunTest(void){
  NumCreated += OS_AddThread(&TestDisk,128,3);  
}
//******************* test main1 **********
// SYSTICK interrupts, period established by OS_Launch
// Timer interrupts, period established by first call to OS_AddPeriodicThread
int TestMainOne(void){   // testmain1
  OS_Init();           // initialize, disable interrupts

//*******attach background tasks***********
  //OS_AddPeriodicThread(&disk_timerproc,10*TIME_1MS,3,1);   // time out routines for disk
  OS_AddSW1Task(&RunTest,3);
