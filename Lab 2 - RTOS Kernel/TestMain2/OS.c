...
void SysTick_Handler(){ tcb* LastThread;
	...
	if(firstThread->sleep == 0){
		if(firstThread->next->sleep == 0 && firstThread->next->priority == firstThread->priority){
...
unsigned long OS_TimeDifference(unsigned long start, unsigned long stop){ int diff;
	if (stop < start){
		diff = !(stop - start) + 1;
	}
